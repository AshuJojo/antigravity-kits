...
To guarantee that *every* Server Action always returns the correct `ActionState` type and never crashes the server due to an unhandled exception (e.g. Prisma throwing a unique constraint violation), define a safe higher-order wrapper.
